...
const colors:Record<string,string>={rack:'#526f67',pallet:'#b98654',crate:'#c28c55',barrier:'#e16a3c',pillar:'#869490',charger:'#397b8d',packing_station:'#657f96',door_frame:'#948b7a'};
...
function RobotBody({robot,pose}:{robot:Robot;pose:[number,number,number]}){
 return <group position={[pose[0],pose[1],0]} rotation={[0,0,pose[2]]}>
  ...
  <mesh castShadow position={[0,0,robot.height/2+.002]}><boxGeometry args={[...robot.footprint,robot.height]}/><meshStandardMaterial color="#e87039" roughness={.5}/><Edges color="#8a331c"/></mesh>
  <mesh position={[.05,0,robot.height+.018]}><boxGeometry args={[robot.footprint[0]*.62,robot.footprint[1]*.7,.025]}/><meshStandardMaterial color="#222f2c"/></mesh>
  <mesh position={[robot.footprint[0]/2+.01,0,robot.height*.6]}><boxGeometry args={[.02,robot.footprint[1]*.55,.05]}/><meshBasicMaterial color="#fff3cf"/></mesh>
 </group>;
}
export default function WorldScene(p:Props){
 ...
 return <Canvas shadows camera={{position:[18,22,22],fov:42,near:.1,far:150}} gl={{antialias:true}}><color attach="background" args={['#e8eeeb']}/><ambientLight intensity={1.5}/><directionalLight position={[8,20,5]} intensity={2.8} castShadow shadow-mapSize={[2048,2048]} shadow-camera-left={-18} shadow-camera-right={18} shadow-camera-top={18} shadow-camera-bottom={-18}/>
 ...
 <mesh receiveShadow onClick={pick}><planeGeometry args={[p.world.bounds.width,p.world.bounds.length]}/><meshStandardMaterial color="#f8faf7" roughness={.95}/></mesh>
 {floorLines.map((line,i)=><Line key={i} points={line.map(([x,y])=>[x,y,.012])} color="#d5dfd9" lineWidth={.6}/>)}
 ...
 {wallMeshes(p.world).map(w=><mesh key={w.id} position={w.position}><boxGeometry args={w.size}/><meshStandardMaterial color="#8ca497" transparent opacity={.28}/><Edges color="#8ca497"/></mesh>)}
 ...
 <mesh castShadow receiveShadow><boxGeometry args={objectMesh(o).size}/><meshStandardMaterial color={p.selected===o.id?'#dc723f':colors[o.type]??'#647b70'} transparent opacity={o.blocking?1:.4}/><Edges color={p.selected===o.id?'#f9dcab':'#354b43'} threshold={15}/></mesh>
 {o.type==='rack'&&[.28,.65].map(h=><mesh key={h} position={[0,-o.footprint[1]/2-.006,(h-.5)*o.height]}><boxGeometry args={[o.footprint[0]-.1,.018,.035]}/><meshStandardMaterial color="#c1d1c6"/></mesh>)}
 </group>)}
