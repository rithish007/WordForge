...
export default function Home(){
 ...
 useEffect(()=>{let active=true;(async()=>{try{
  ...
  const v=await api<Validation>('validate',{world:ws[0],robot_id:'rb_theron'});
  if(active){setWorlds(ws);setRobots(rs);setWorld(ws[0]);setValidation(v);setHasSaved(!!localStorage.getItem('worldforge.world.v1'));}
 ...
 async function toggleBarrier(){if(!world)return;const exists=world.objects.some(o=>o.id==='cross_aisle_barrier');
  await apply({...world,objects:exists?world.objects.filter(o=>o.id!=='cross_aisle_barrier'):[...world.objects,{id:'cross_aisle_barrier',label:'Cross aisle barrier',type:'barrier',center:[0,0],footprint:[world.bounds.width,.3],height:1.1,yaw_deg:0,blocking:true}]},exists?'Cross-aisle reopened.':'Barrier added. Check the route to see what changed.');}
 return <main>
 <header className="topbar"><a className="brand" href="/"><span className="brand-mark">W</span>WorldForge<span className="brand-divider">/</span><span className="brand-sub">Warehouse lab</span></a><div className="top-right"><span className="mode-dot"/> Local prototype <a href="https://worldforge-nine.vercel.app/" target="_blank" rel="noreferrer" className="text-link">Submission ↗</a></div></header>
 ...
 <div className="button-row"><button disabled={!!busy} onClick={()=>input.current?.click()}>↥ Import</button><button disabled={!world||!!busy} onClick={()=>world&&download(world.id+'.json',world)}>↧ Export</button></div>
 ...
 <div className="objects">{world?.objects.map(o=><button key={o.id} className={'object-row '+(selected===o.id?'selected':'')} disabled={!!busy} onClick={()=>setSelected(o.id)}><span className={'object-dot '+o.type}/><span>{o.label}</span><span className="object-arrow">↗</span></button>)}</div>
 ...
 {world&&robot?<WorldScene world={world} robot={robot} start={start} goal={goal} path={route?.path??[]} frame={frame} selected={selected} onSelect={setSelected} onFloor={setPoint} grid={validation?.grid??null} showGrid={showGrid} pickMode={pick}/>:<div className="scene-loading">{error?'Local simulator is offline.':'Loading your workspace…'}</div>}
 <div className="viewport-top"><span className="viewport-chip">PERSPECTIVE · Z UP</span><label className="overlay-toggle"><input type="checkbox" checked={showGrid} onChange={e=>setShowGrid(e.target.checked)}/> Clearance map</label></div>
 ...
 <section className="results"><div className="results-heading"><div><span className="section-kicker">RUN RESULTS</span><h2>{result?.metrics?'Measured in motion':'The route is only the beginning.'}</h2></div><span className="physics-label">Planar MuJoCo physics</span></div>
